...
export const computeLeavesNeeded = (endpointCount, portsPerLeaf) => (portsPerLeaf <= 0 || endpointCount <= 0) ? 0 : Math.ceil(endpointCount / portsPerLeaf);
export const computeSpinesNeeded = (leaves, uplinksPerLeaf) => (leaves <= 0 || uplinksPerLeaf <= 0) ? 0 : Math.max(1, Math.ceil((leaves * uplinksPerLeaf) / 32));
export const computeOversubscription = (uplinks, downlinks) => uplinks <= 0 ? 0 : downlinks / uplinks;
export const validateFabricSpec = (spec) => {
    try {
...
export const computeTopology = (spec) => {
...
export const generateWiringStub = (fabric, topology) => {
