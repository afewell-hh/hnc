 ...
 */
export async function detectDrift(fabricId, currentDiagram, options = {}) {
 ...
 */
export function generateDriftReport(current, onDisk, options = {}) {
    const startTime = Date.now();
    const changes = [];
    // Compare switches (spines and leaves)
    const switchChanges = compareDevices([...current.devices.spines, ...current.devices.leaves], [...onDisk.devices.spines, ...onDisk.devices.leaves], 'switch');
    changes.push(...switchChanges);
    // Compare endpoints/servers
    const endpointChanges = compareDevices(current.devices.servers, onDisk.devices.servers, 'endpoint');
 ...
 */
function compareDevices(current, onDisk, category) {
    const changes = [];
 ...
 */
function compareConnections(current, onDisk) {
    const changes = [];
    // Create comparable keys for connections
    const getConnectionKey = (conn) => `${conn.from.device}:${conn.from.port}->${conn.to.device}:${conn.to.port}`;
 ...
 */
function categorizeDriftChanges(changes) {
    return {
 ...
 */
function buildDriftSummaryStrings(changes) {
    if (changes.length === 0) {
...
function findObjectDifferences(current, disk) {
    const differences = [];
...
function estimateDiagramSize(diagram) {
