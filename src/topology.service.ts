import { FabricConfig, ComputedTopology } from '../types'
import { CatalogService } from './catalog.service'

export class TopologyService {
  static computeTopology(config: FabricConfig): ComputedTopology {
    const errors: string[] = []
    ...
    if (!spineModel) errors.push(`Invalid spine model: ${config.spineModelId}`)
    if (!leafModel) errors.push(`Invalid leaf model: ${config.leafModelId}`)
  ...
  static generateWiringDiagramStub(topology: ComputedTopology, config: FabricConfig) {
    if (!topology.isValid) return null
