 ...
 */
export function serializeWiringDiagram(diagram) {
    // Servers YAML - includes both spines and leaves, plus endpoint servers
    const allSwitches = [
        ...diagram.devices.spines.map(s => ({ ...s, type: 'spine' })),
        ...diagram.devices.leaves.map(l => ({ ...l, type: 'leaf' }))
    ...
    const connectionsData = {
        connections: diagram.connections.sort((a, b) => a.from.device.localeCompare(b.from.device) ||
 ...
 */
export function deserializeWiringDiagram(yamls) {
    try {
        const serversData = yaml.load(yamls.servers);
        const switchesData = yaml.load(yamls.switches);
        const connectionsData = yaml.load(yamls.connections);
        ...
        const spines = allSwitches.filter((s) => s.type === 'spine')
            .map(({ type, ...rest }) => rest);
        const leaves = allSwitches.filter((s) => s.type === 'leaf')
            .map(({ type, ...rest }) => rest);
