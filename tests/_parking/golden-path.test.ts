...
const goldenPathSpec: FabricSpec = {
...
describe('Golden Path: Compute → Save → Reload', () => {
  ...
  it('should complete full compute→save→reload workflow successfully', async () => {
    ...
    expect(loadedDiagram.metadata.totalDevices).toBe(
      topology.spinesNeeded + topology.leavesNeeded + goldenPathSpec.endpointCount
    )
  ...
  it('should handle invalid configurations gracefully', async () => {
    const invalidSpec: Partial<FabricSpec> = {
      name: 'invalid-fabric',
      spineModelId: 'DS3000',
      leafModelId: 'DS2000',
